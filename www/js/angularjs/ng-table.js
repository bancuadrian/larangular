!
    function(a, b) {
        ...
        b.factory("ngTableParams", ["$q", function(b) {
            var c = function(a) {
                ...
                d = function(d, e) {
                    ...
                    this.parameters = function(b, d) {
                        ...
                        b = b || !1;
                        var c = b ? [] : {};
                        for (key in g) if (g.hasOwnProperty(key)) {
                            var d = g[key],
        ...
        return b.directive("ngTable", ["$compile", "$q", "$parse", function(b, d, e) {
            ...
            a.put(
                ...
                '<select '+
                    ...
                    '</select>'),
                a.put("ng-table/filters/text.html", '<input type="text" ng-model="params.filter()[name]" ng-if="filter==\'text\'" class="input-filter form-control"/>'),
                ...
                a.put("ng-table/pager.html", '<div ng-if="params.$params.showPagination"><div class="ng-cloak"> <div ng-if="params.settings().counts.length" class="ngitemspp"><div ng-if="params.$params.showCounter"><select ui-select2="{ minimumResultsForSearch:99 }" ng-change="params.count(params.$params.count)" ng-model="params.$params.count"><option ng-repeat="count in params.settings().counts" ng-class="{\'active\':params.count()==count}" value="{{count}}">{{count}}</option></select></div></div> <ul class="pagination"> <li ng-class="{\'disabled\': !page.active}" ng-repeat="page in pages" ng-switch="page.type"> <a id="prevpag" ng-switch-when="prev" ng-click="params.page(page.number)" href=""><span></span>Previous</a> <a ng-switch-when="first" ng-click="params.page(page.number)" href="">{{page.number}}</a> <a ng-switch-when="page" ng-click="params.page(page.number)" href="">{{page.number}}</a> <a class="manypagesdotted" ng-switch-when="more" ng-click="params.page(page.number)" href="">&#8230;</a> <a ng-switch-when="last" ng-click="params.page(page.number)" href="">{{page.number}}</a> <a id="nextpag" ng-switch-when="next" ng-click="params.page(page.number)" href="">Next<span></span></a> </li> </ul> </div></div>')
        }]), b
    });
